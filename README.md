...
CORE is intentionally storage-agnostic. A user or team can place the structure wherever it is most useful, including SharePoint, Google Drive, Dropbox, Git, OneDrive, a local folder, or a ChatGPT project context.
...
CORE separates the pattern from the mechanisms that may support it:
...
CORE can be used in several ways depending on available tools, connectors, and permissions.
...
If a chatbot has access to a storage connector, such as SharePoint, Google Drive, Dropbox, or Git, it may be able to read from the CORE folder. If the connector also has write permission, the chatbot may be able to create or update files in the structure.

### 3. Session context mode

The structure does not always need to be physically written back to storage. A chatbot can use the CORE structure and instructions as an in-session working model, keeping track of sources, notes, and outputs during the active session. In this mode, the structure guides the work, but changes are only preserved if the user saves them or a connector writes them back.
...
CORE does not provide RAG, file access, or write-back capability by itself. It provides the structure and instructions that a chatbot, connector, or retrieval workflow can use.
...
CORE helps users create a bounded project context. The user can fill that context manually, use a chatbot to help organize it, or connect it to tools that read and write files. The same structure can be used lightly in a chat session or more formally in a shared project folder.
